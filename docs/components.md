...
This component mounts when the member dashboard page loads. It makes a GET request to [/api/member/health-metrics](/app/app/api/member/health-metrics/route.ts) and lists every metric the logged in member has logged. Each list item has a button to delete the metric which makes a DELETE request to [/api/member/health-metrics/{metricId}](/app/app/api/member/health-metrics/[metricId]/route.ts). Pressing the "Log New Metric" button in this section opens the LogHealthMetricModal component.
