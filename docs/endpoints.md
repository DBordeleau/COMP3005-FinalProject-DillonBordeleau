...
- [/classes/route.ts](/app/app/api/admin/classes/route.ts) - CRUD endpoint for [GroupClass](/app/models/GroupClass.ts) entities. Interacted with via the [ClassList](/app/app/components/ClassList.tsx) component on the admin dashboard.
- [/rooms/route.ts](/app/app/api/admin/rooms/) - CRUD endpoint for [Room](/app/models/Room.ts) entities. Also contains an endpoint to check room availability. Interacted with via the [RoomsList](/app/app/components/RoomsList.tsx) component on the admin dashboard.
    - [/rooms/route.ts](/app/app/api/admin/rooms/route.ts) - CRUD endpoint for [Room](/app/models/Room.ts) entities.
...
- [/fitness-goals/route.ts](/app/app/api/member/fitness-goals/route.ts) - CRUD endpoint for [FitnessGoal](/app/models/FitnessGoal.ts) entities. Fitness goals can't be edited but members can change their status by marking them as achieved/failed. Interacted with via the [FitnessGoalsSection](/app/app/components/FitnessGoalsSection.tsx) component on the member dashboard.
- [/health-metrics/route.ts](/app/app/api/member/health-metrics/route.ts) - Creation, Read and Delete endpoint for [HealthMetric](/app/models/HealthMetric.ts) entities. [HealthMetricSection](/app/app/components/HealthMetricSection.tsx) component on the member dashboard.
- [/training-sessions/](/app/app/api/member/training-sessions/route.ts) - CRUD endpoint for [TrainingSession](/app/models/TrainingSession.ts) entities.
- [/group-classes/](/app/app/api/member/group-classes/) - CRUD endpoint for [ClassEnrollment](/app/models/ClassEnrollment.ts) entities. Enrolls the logged in user in a given group class on a POST submission as long as they are not already registered and capacity isn't full. Inserts trigger the check_class_capacity() function defined in [CREATE_TRIGGER.ts](/app/migrations/CREATE_TRIGGER.ts).
